...
## `product.proto` — Danh mục sản phẩm
...
## `recommend.proto` — Gợi ý & so sánh sản phẩm
...
::: tip Điểm chung giữa `recommend.proto` và `docs.proto`
...
## Vì sao 3 package nằm phẳng ở root thay vì `techscout/<svc>/v1/`?

Xem comment trong [`buf.yaml`](https://github.com/nxhawk/techscout-protos/blob/main/buf.yaml):
đây là lựa chọn có chủ đích để giữ đường dẫn submodule và các flag `-I` của
`protoc` không đổi ở tất cả service, đánh đổi lấy việc tắt 2 rule lint
(`DIRECTORY_SAME_PACKAGE`, `PACKAGE_DIRECTORY_MATCH`) vốn yêu cầu cấu trúc thư
mục khớp với package.
