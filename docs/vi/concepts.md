...
[Cập nhật proto](/vi/updating-protos). Muốn xem **workflow CI** chạy gì → xem
[Luồng CI/CD](/vi/ci-flow). Muốn tra **từng RPC/message** → xem
[Tham chiếu proto](/vi/proto-reference).
:::
...
[Tham chiếu proto](/vi/proto-reference). Cả 3 hiện đều dùng **unary RPC**
...
| Cấu hình trong repo | `buf.yaml` → `lint.use: STANDARD`, trừ 2 rule tắt (xem [Tham chiếu proto](/vi/proto-reference)) | `buf.yaml` → `breaking.use: FILE` |
...
- **CI chạy hộ** trong `proto-sync.yml` của từng repo mỗi khi nhận
  ...
  [Luồng CI/CD](/vi/ci-flow#3-phía-consumer-proto-guardyml-proto-syncyml)).
:::
...
(kèm checklist) xem
[Quy trình cập nhật proto § Thêm version mới](/vi/updating-protos#thêm-version-mới-v2-song-song-với-v1).
...
[Quy trình cập nhật proto](/vi/updating-protos#thêm-version-mới-v2-song-song-với-v1)
...
→ xem trọn [Quy trình cập nhật proto](/vi/updating-protos). Chuỗi sự kiện CI
đầy đủ (workflow nào chạy khi nào) → xem [Luồng CI/CD](/vi/ci-flow).
...
lý do toàn bộ hạ tầng CI trong [Luồng CI/CD](/vi/ci-flow) tồn tại: đảm bảo mọi
